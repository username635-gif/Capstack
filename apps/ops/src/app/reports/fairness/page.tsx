...
import type { FairnessReport, FairnessPeriod, FairnessAdviserRow } from "@capstack/types";
    ...
    scoreBandDistribution: [
      {
        ...
        count: 180,
        approvalRate: 95.0,
        predictedDefaultRate: 1.2,
        actualDefaultRate: 1.0,
      },
      {
        band: "B",
        count: 320,
        approvalRate: 82.0,
        predictedDefaultRate: 3.5,
        actualDefaultRate: 3.8,
      },
      {
        band: "C",
        count: 410,
        approvalRate: 65.0,
        predictedDefaultRate: 6.8,
        actualDefaultRate: 7.1,
      },
      {
        band: "D",
        count: 280,
        approvalRate: 38.0,
        predictedDefaultRate: 12.0,
        actualDefaultRate: 11.4,
      },
      {
        band: "E",
        count: 145,
        approvalRate: 12.0,
        predictedDefaultRate: 22.0,
        actualDefaultRate: 24.1,
      },
    ],
    overrideAnalysisByAdviser: [
      {
        adviserId: "1",
        adviserName: "T. Mokoena",
        totalDecisions: 145,
        overrideCount: 18,
        overrideRate: 12.4,
        overrideApprovalRate: 72.0,
        overrideDefaultRate: 4.2,
        flagged: false,
      },
      {
        adviserId: "2",
        adviserName: "S. Dlamini",
        totalDecisions: 98,
        overrideCount: 24,
        overrideRate: 24.5,
        overrideApprovalRate: 68.0,
        overrideDefaultRate: 6.8,
        flagged: true,
      },
      {
        adviserId: "3",
        adviserName: "A. Pieterse",
        totalDecisions: 112,
        overrideCount: 14,
        overrideRate: 12.5,
        overrideApprovalRate: 78.0,
        overrideDefaultRate: 3.1,
        flagged: false,
      },
    ],
            ...
            <ResponsiveContainer width="100%" height={340}>
              ...
              >
                ...
                <Bar dataKey="approvalRate" isAnimationActive={false}>
                  ...
                  <LabelList dataKey="approvalRate" position="right" formatter={pctFormatter} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr style={{ background: "var(--color-surface-2)" }}>
                  <th style={{ border: "0.5px solid var(--color-border)" }} className="px-3 py-2">
                    Band
                  </th>
                  <th style={{ border: "0.5px solid var(--color-border)" }} className="px-3 py-2">
                    Applications
              ...
              <tbody>
                {data.scoreBandDistribution.map((row) => {
                  ...
                  return (
                    <tr key={row.band}>
                      <td style={{ border: "0.5px solid var(--color-border)" }} className="px-3 py-2 text-center">
                        {row.band}
                      </td>
                      <td style={{ border: "0.5px solid var(--color-border)" }} className="px-3 py-2 text-center">
                        {row.count}
                      ...
                      <td style={{ border: "0.5px solid var(--color-border)" }} className="px-3 py-2 text-center">
                        {pctFormatter(row.approvalRate)}
                      </td>
                      <td style={{ border: "0.5px solid var(--color-border)" }} className="px-3 py-2 text-center">
                        {pctFormatter(row.predictedDefaultRate)}
                      </td>
                      <td style={{ border: "0.5px solid var(--color-border)" }} className="px-3 py-2 text-center">
                        {pctFormatter(row.actualDefaultRate)}
                      </td>
                      <td
                        style={{ border: "0.5px solid var(--color-border)" }}
                        className={`px-3 py-2 text-center ${varianceColor}`}
                      >
                        {variance > 0 ? "+" : ""}
                ...
                {data.overrideAnalysisByAdviser
                  ...
                  .map((row) => (
                    <tr key={row.adviserId}>
                      <td style={{ border: "1px solid var(--color-border)" }} className="px-3 py-2 text-center">
                        {row.adviserName}
                      </td>
                      <td style={{ border: "1px solid var(--color-border)" }} className="px-3 py-2 text-center">
                        {row.totalDecisions}
