 ...
 *
 * A slide-over modal triggered by clicking the calculator icon in the nav.
 * Shows real-time payment estimates and an affordability verdict.
 *
 * CALCULATIONS:
 ...
 *   net monthly income (configurable). Shows a clear AFFORD / TIGHT / OVER verdict.
 *
 * FIELDS:
 *   Loan amount (slider + input)
 *   Term in months (slider + input)
 *   Annual interest rate (pre-filled, editable)
 *   Monthly net income (for affordability check)
 *   Monthly expenses (optional)
 *
 * No login required — purely client-side math.
 */
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Sliders */}
        <Input label="Loan amount"   value={amount}  onChange={setAmount}  min={1000}  max={500000} step={1000} prefix="R " />
        <Input label="Term"          value={months}  onChange={setMonths}  min={1}     max={84}                 suffix=" months" />
        <Input label="Interest rate" value={ratePct} onChange={setRatePct} min={5}     max={30}     step={0.5}  suffix="% p.a." />
            ...
            <div>
              ...
              {verdict !== 'UNKNOWN' && (
                <div style={{ fontSize: 11, color: verdictMeta.color, marginTop: 2 }}>
                  {verdict === 'AFFORDABLE'
                    ? `Installment is ${((monthly / (income - expenses)) * 100).toFixed(0)}% of disposable income (below 30% guideline)`
                    : verdict === 'TIGHT'
                      ? `Installment is ${((monthly / (income - expenses)) * 100).toFixed(0)}% of disposable income (30–45% — proceed with caution)`
                      : `Installment exceeds 45% of disposable income — NCA affordability likely to fail`}
                </div>
              )}
            </div>
          </div>
        ...
        <p style={{ fontSize: 10, color: 'var(--color-muted)', textAlign: 'center', marginTop: 10 }}>
          This calculator is for illustration only. Final rates and approval depend on your credit assessment.
          All amounts are estimates per NCA s.92 disclosure requirements.
        </p>
      </div>
    </div>
  );
...
// ─── Calculator SVG icon (thin white / inherited lines) ───────────────────────
...
export function CalculatorIcon({
  ...
  return (
    ...
    >
      ...
      {/* Button grid — bottom row (= spans 2) */}
